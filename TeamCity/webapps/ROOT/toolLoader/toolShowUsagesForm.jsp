...
<c:choose>
  ...
  <c:otherwise>
    Tool is used in <strong>${affectedConfigurationsCount}</strong> <bs:plural txt="build configuration" val="${affectedConfigurationsCount}" />.
    <c:if test="${not empty explicitUsages}">
      <c:set var="explicitUsagesCount" value="${fn:length(explicitUsages)}"/>
      <div class="btUsages">
        <div>
          Used as a <b>concrete</b> version in <b>${explicitUsagesCount}</b> build configuration<bs:s val="${explicitUsagesCount}"/>:
        </div>
    ...
    <c:if test="${not empty defaultUsages}">
      ...
      <div class="btUsages">
        <div>
          Used as a <b>default</b> version in <b>${defaultUsagesCount}</b> build configuration<bs:s val="${defaultUsagesCount}"/>:
        </div>
        <ul class="btUsages__list">
