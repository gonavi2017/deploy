...
<jsp:useBean id="outdated" scope="request" type="java.lang.Boolean"/>
...
<bs:externalPage>
  <jsp:attribute name="page_title">TeamCity: User Status</jsp:attribute>
  ...
  <jsp:attribute name="body_include">
    ...
    <div class="panel">
      ...
      <c:if test="${outdated == true}">
        <div class="notifierOutdated">
          <p>Notifier version does not match server version.
            Please download the new version and update the program.
            <a href="../update/TrayNotifierInstaller.msi" target="_blank" onclick="return Win32.Extension.downloadInstallUpdate();">Download installer</a>
          </p>
        </div>
      </c:if>
      ...
      <c:if test="${not hasRules}">
        <p class="messageEmpty">You do not monitor any build configurations or there are no build configurations at all.
          <a href="<c:url value="/profile.html?tab=userNotifications&notificatorType=WindowsTray"/>" target="_blank">Edit settings</a>
        </p>
      </c:if>
