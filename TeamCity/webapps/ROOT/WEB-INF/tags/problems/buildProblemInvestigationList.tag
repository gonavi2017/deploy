...
<c:if test="${not empty projectBuildProblemsBean and projectBuildProblemsBean.buildProblemsNumber > 0}">
  ...
  <c:set var="title"><span class="icon icon16 bp taken"></span> Build problem investigations</c:set>
  <bs:_collapsibleBlock title="${title}" id="buildProblemInvestigations" collapsedByDefault="false">

    <problems:buildProblemExpandCollapse showExpandCollapseActions="true">
        <problems:buildProblemGroupByProject projectBuildProblemsBean="${projectBuildProblemsBean}" inlineResponsibiltyInfo="${true}"/>
