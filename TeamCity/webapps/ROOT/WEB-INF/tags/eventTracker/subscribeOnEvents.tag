<%@ tag import="jetbrains.buildServer.serverSide.tracker.EventTracker"
  ...
  %><jsp:useBean id="__tc_eventTracker" type="jetbrains.buildServer.serverSide.tracker.EventTracker" scope="request"
  ...
  /><c:set var="eventsNumber" value='<%=__tc_eventTracker.getNumberOfEvents((TrackerEventType)jspContext.getAttribute("event"))%>'
  />BS.EventTracker.subscribeOnEvent('${event.shortCode}', ${eventsNumber}, handler);</c:if
  ></c:forEach>})();</script>