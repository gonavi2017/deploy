...
function getSessionNew(TypeScriptSession, TypeScriptProjectService, TypeScriptCommandNames, logger, host, ts_impl, defaultOptionsHolder, mainFile, projectEmittedWithAllFiles) {
    ...
    if (util_1.isVersionMoreOrEqual(tsVersion, 2, 3, 1)) {
        var options = {
            ...
            useSingleInferredProject: isUseSingleInferredProject,
        ...
        session = new IDESessionImpl(host, cancellationToken, isUseSingleInferredProject, nullTypingsInstaller, Buffer.byteLength, process.hrtime, logger, true);
    }
    return session;
}
